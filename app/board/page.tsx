...
export default function Board() {
  ...
  const { currentItems, currentPage, pageNum, lastPage, goToPage, goToNextPage, goToPreviousPage } = usePagination(
    getJson,
    5,
  );
          ...
          <section className={style.container_content_left_bottom}>
            <section className={style.search_box}>
              ...
              <div className={style.search_box_bottom}>
                <Button
                  type={'tag'}
                  typeStyle={activeBtn === 'latest' ? 'active' : ''}
                  onClick={() => {
                    handleChangeActive('latest');
                  }}
                  text={'최신순'}
                />
                <Button
                  type={'tag'}
                  typeStyle={activeBtn === 'popular' ? 'active' : ''}
                  onClick={() => {
                    handleChangeActive('popular');
                  }}
                  text={'인기순'}
                />
              </div>
            ...
            <div className={style.button_container}>
              ...
              {new Array(5).fill(1).map((_, idx) => {
                return (
                  idx + pageNum['current'] <= lastPage['current'] && (
                    <Button
                      key={idx + pageNum['current']}
